...
export class SigninComponent {
  ...
  onSignIn(){
    this.toast.showToast.set(true)
    this.toast.toastContent.set("Logged in successfully")
    setTimeout(()=>{
      this.toast.showToast.set(false)
      this.toast.toastContent.set("")
    },5000)

    console.log(this.signinForm.status)

    this.auth.signin({email: this.signinForm.value.email as string, password: this.signinForm.value.password as string}).subscribe(res=>{

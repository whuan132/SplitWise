      ...
      <div class="max-w-2xl mx-auto">
        ...
        <div class="flex items-center space-x-2 justify-between ">
          ...
          <div class="flex space-x-2">
              ...
              <div class="flex" *ngFor="let category of categories">
                  <button type="button"
                          ...
                          (click)="filter(category)">{{category | titlecase}}</button>
          ...
          <div>
              ...
              <div class="flex items-center">
                  <label for="category"
                         class="block  text-sm font-medium text-gray-900 dark:text-white">sortby:</label>
                  <select id="category" name="category"
                          ...
                          (change)="sortTransactions(Sort.value)" #Sort>
        ...
        <hr class="my-4"/>
          <ol class="relative border-l border-gray-200 dark:border-gray-700">
              <li class="mb-10 ml-6" *ngFor="let transaction of (transactions_filter || transactions)">
        <span class="absolute flex items-center justify-center w-6 h-6 bg-blue-100 rounded-full -left-3 ring-8 ring-white dark:ring-gray-900 dark:bg-blue-900">
            ...
            <svg aria-hidden="true" class="w-3 h-3 text-blue-800 dark:text-blue-300" fill="currentColor"
                 viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd"
                  ...
                  <h3 class="flex items-center  text-lg font-semibold text-gray-900 dark:text-white">{{transaction.title}}
                      <span class="bg-green-100 text-green-800 text-sm font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-green-900 dark:text-green-300 ml-3 cursor-pointer" (click)="filter(transaction.category)">{{transaction.category }}</span>
                  </h3>
                  <time class="block mb-2 text-sm font-normal leading-none text-gray-400 dark:text-gray-500">Paid
                      by {{transaction.paid_by.fullname}} on {{transaction.date | date}}</time>
                  <p class="mb-4 text-xl  font-bold text-gray-500 dark:text-gray-400 line-clamp-2" *ngIf="transaction.category==='PAYBACK'">{{transaction.amount | currency}}</p>
                  <p class="mb-4 text-base font-normal text-gray-500 dark:text-gray-400 line-clamp-2">{{transaction.description}}</p>
                  <div class="flex items-center space-x-8">
                      <a href="#"
                         class="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:outline-none focus:ring-gray-200 focus:text-blue-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700 dark:focus:ring-gray-700">
                          ...
                          Download Receipt</a>
                      <button class=" bg-white border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 font-medium rounded-lg text-sm px-5 py-2 mr-2  dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700 text-red-500"
                              *ngIf="state.user()._id===transaction.paid_by.user_id"
                              (click)="removeTransaction(transaction._id)">remove
                      </button>
                  </div>
              </li>
